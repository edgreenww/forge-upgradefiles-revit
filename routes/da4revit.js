...
router.post('/da4revit/v1/upgrader/files', async (req, res, next) => {
    ...
    console.log('upgradeRequestBody', req.body)
    ...
    console.log("req.oauth_token", req.oauth_token) // <-- this 
    ...
    try {
        ...
        console.log(items)
        const folder = await items.getItemParentFolder(projectId, resourceId, oauth_client, oauth_token);
        if(folder === null || folder.statusCode !== 200){
            console.log('failed to get the parent folder.');
            res.status(500).send('ailed to get the parent folder');
            return;
        }
        ...
        if( fileExtension !== 'rvt' && fileExtension !== 'rfa' && fileExtension !== 'fte'){
            ...
            res.status(500).send('the file format is not supported');
            return;
        }

        console.log("fileParams", fileParams)

        const storageInfo = await getNewCreatedStorageInfo(projectId, folder.body.data.id, fileItemName, oauth_client, oauth_token);
        if (storageInfo === null ) {
            console.log('failed to create the storage');
            res.status(500).send('failed to create the storage');
            return;
        }
        ...
        const versionInfo = await getLatestVersionInfo(projectId, resourceId, oauth_client, oauth_token);
        if (versionInfo === null ) {
            console.log('failed to get lastest version of the file');
            res.status(500).send('failed to get lastest version of the file');
            return;
        }
        ...
        if (createVersionBody === null ) {
            ...
            res.status(500).send('failed to create body of Post Version');
            return;
        }

        ////////////////////////////////////////////////////////////////////////////////
        // use 2 legged token for design automation
        const oauth2 = new OAuth(req.session);
        console.log("oauth2", oauth2)
        const oauth2_client = oauth.get2LeggedClient(
            client_id = req.body.client_id,
            client_secret= req.body.client_secret,
        );

        //const oauth_client = req.oauth_client
        const oauth2_token = await oauth2_client.authenticate();

        console.log(inputUrl, outputUrl, projectId, createVersionBody, fileExtension, oauth_token , oauth2_token)


        let upgradeRes = await upgradeFile(inputUrl, outputUrl, projectId, createVersionBody, fileExtension, oauth_token, oauth2_token );
        if(upgradeRes === null || upgradeRes.statusCode !== 200 ){
            console.log('failed to upgrade the revit file');
            res.status(500).send('failed to upgrade the revit file');
            return;
        }
        ...
        res.status(200).send(JSON.stringify(upgradeInfo));
